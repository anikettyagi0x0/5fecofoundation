...
const FloatingSocials = () => {
  ...
  // We wait until the component is mounted on the client before teleporting it
  useEffect(() => {
    setMounted(true);
  }, []);
  ...
  // If it's rendering on the server, return nothing to prevent errors
  if (!mounted) return null;

  // TELEPORT the UI directly into the <body> tag, bypassing all wrappers
  return createPortal(
    <div className="fixed right-0 top-1/2 -translate-y-1/2 z-[999999]">
    ...
    document.body // <-- This is where the magic happens
  );
};
